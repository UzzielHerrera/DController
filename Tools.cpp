...
// Tools::Tools(){
void Tools::begin(void) {
  Serial.println("Tools Module initialized.");
}
...
void Tools::PrintPoint(Point point, String pointName){
  Serial.println("Point " + pointName + ": [ " + String(point.x, 2) +", " + String(point.y, 2) + ", " + String(point.z, 2) +"]");
}

void Tools::PrintMoveDesigner(MoveDesigner md, String moveDesignerName){
  Serial.print("MoveDesigner " + moveDesignerName + ": {");
  for(uint8_t i = 0; i < 3; i++){
    Serial.print(" " + String(md.Designer[i].position) + " ");
  }
  Serial.print("} for ");
  Serial.println(md.moveDuration);
}

void Tools::PrintAngle(Angle angle, String angleName){
  Serial.print("Angle " + angleName + ": ");
  Serial.print("{Theta1: " + String(angle.theta1));
  Serial.print(",Theta2: " + String(angle.theta2));
  Serial.println(",Theta3: " + String(angle.theta3)+"}");
}
...
void Tools::PrintVector(Vector vector, String vectorName){
  Serial.println("Vector " + vectorName + ": [ " + String(vector.x, 2) +", " + String(vector.y, 2) + ", " + String(vector.z, 2) +"]");
}
...
void Tools::setSleepTime(uint32_t time){
  data.sleepTime = time;
